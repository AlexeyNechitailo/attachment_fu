    ...
    module Backends
      ...
      module S3v2Backend
        ...
        def self.included(base) #:nodoc:
          ...
          begin
            @@s3_config_path = base.attachment_options[:s3_config_path] || (Rails.root.join('config/amazon_s3_tracks.yml'))
        ...
        protected
          ...
          def rename_file
            ...
            old_full_filename.sub!(/^\/{1}/, '') #remove first '/' symbol

            s3_conn.buckets[bucket_name].objects[old_full_filename].move_to(full_filename, :acl => attachment_options[:s3_access]) if s3_conn.buckets[bucket_name].objects[old_full_filename].exists?
